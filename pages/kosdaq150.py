...
def run_analysis(skip_daily: bool):
    ...
    for code in result["new_selected"]:
        ...
        r = info.iloc[0]
        status = "유지"
        if code not in set(current_150):
            status = "신규편입"
...
st.sidebar.markdown("---")
st.sidebar.caption(
    "KRX 방법론 기반 코스닥 150 구성종목\n편입/편출 예측 시스템\n\n"
    "- 빠른 분석: 당일 시가총액/거래대금 기준\n"
    "- 정밀 분석: 6개월 일평균 데이터 기준 (시간 소요)"
)
...
if run_button or "kosdaq150_analysis" in st.session_state:
    ...
    with tab1:
        ...
        with col_pie:
            fig = px.pie(cur_sector, values="종목수", names="섹터",
                         title="섹터별 종목 수 비중", hole=0.4, height=400)
            fig.update_traces(textposition="inside", textinfo="label+percent")
            st.plotly_chart(fig, use_container_width=True)
        with col_bar:
            fig = px.bar(cur_sector.sort_values("시가총액(조)", ascending=True),
                         x="시가총액(조)", y="섹터", orientation="h",
                         title="섹터별 시가총액 (조원)",
                         color="시가총액(조)", color_continuous_scale="Blues", height=400)
        ...
        fig_tree = px.treemap(tree_df, path=["섹터", "종목명"], values="시가총액(억)",
                              color="섹터", title="시가총액 Top 30 트리맵", height=500)
        st.plotly_chart(fig_tree, use_container_width=True)

        # 종목 테이블
        st.markdown("#### 전체 종목 리스트")
    ...
    # ──────────────────────────────────────
    with tab2:
        st.markdown("### 향후 예상 코스닥 150 구성종목")
        st.caption("KRX 방법론 시뮬레이션 기반 예측 결과입니다. 신규편입 종목은 강조 표시됩니다.")
        ...
        fig_compare.add_trace(go.Bar(
            ...
            name="현재", marker_color="#74b9ff",
        ))
        fig_compare.add_trace(go.Bar(
            x=compare["섹터"], y=compare["예상"],
            name="예상", marker_color="#0984e3",
        ))
        fig_compare.update_layout(barmode="group", title="섹터별 종목 수 (현재 vs 예상)",
                                  height=400)
        ...
        def highlight_new(row):
            if row["상태"] == "신규편입":
                return ["background-color: #0984e3; color: #ffffff"] * len(row)
            return [""] * len(row)
    ...
    with tab3:
        ...
        if not additions:
            ...
            for s in additions:
                ...
                if overall_rank <= 50:
                    reasons.append(f"코스닥 전체 시가총액 {overall_rank}위 (대형주 특례 가능)")
                if sector_rank <= new_threshold:
                    reasons.append(
                        f"섹터 내 시총 {sector_rank}위 "
                        f"(신규편입 기준 {new_threshold}위 이내 충족)"
                    )
                if not reasons:
                    reasons.append(
                        f"3차 선정에서 잔여 종목 중 시총 상위로 150종목 충원"
                    )
            ...
            with col_a1:
                # 편입 종목 시가총액 순위 분포
                fig = px.bar(
                    add_detail_df.sort_values("시가총액", ascending=False).head(20),
                    x="종목명", y="전체순위",
                    color="섹터",
                    title="편입 예상 종목의 전체 시총 순위 (낮을수록 상위)",
                    height=450,
                )
            ...
            with col_a2:
                ...
                fig = px.bar(sec_add.sort_values("편입종목수", ascending=True),
                             x="편입종목수", y="섹터", orientation="h",
                             title="섹터별 신규 편입 종목 수",
                             color="편입종목수", color_continuous_scale="Greens",
                             height=450)
                fig.update_layout(coloraxis_showscale=False)
                st.plotly_chart(fig, use_container_width=True)

            # 편입 사유 상세 테이블
            st.markdown("#### 종목별 편입 사유 상세")
            ...
            for i, row in add_detail_df.head(10).iterrows():
                with st.expander(
                    ...
                    cc1.metric("전체 시총 순위", make_rank_badge(row["전체순위"]))
                    cc2.metric("섹터 내 순위",
                               f"{row['섹터내순위']}위 / {row['섹터기존종목수']}종목 중")
        ...
        if not removals:
            ...
            for s in removals:
                ...
                if sector_rank > keep_threshold:
                    reasons.append(
                        f"섹터 내 시총 {sector_rank}위 "
                        f"(기존유지 기준 {keep_threshold}위 이내 미충족)"
                    )
                if not liquidity_ok:
                    reasons.append(
                        f"유동성 미달 — 섹터 내 거래대금 {amount_rank_in_sector}위 "
                        f"(기준 {liquidity_threshold}위 이내)"
                    )
                if overall_rank > 300:
            ...
            with col_r1:
                fig = px.bar(
                    rem_detail_df.sort_values("전체순위", ascending=True),
                    x="종목명", y="전체순위",
                    color="섹터",
                    title="편출 예상 종목의 전체 시총 순위 (높을수록 하위)",
                    height=450,
                )
                st.plotly_chart(fig, use_container_width=True)
            ...
            with col_r2:
                ...
                fig = px.bar(sec_rem.sort_values("편출종목수", ascending=True),
                             x="편출종목수", y="섹터", orientation="h",
                             title="섹터별 편출 종목 수",
                             color="편출종목수", color_continuous_scale="Reds",
                             height=450)
                fig.update_layout(coloraxis_showscale=False)
                st.plotly_chart(fig, use_container_width=True)

            # 편출 사유 분류
            st.markdown("#### 편출 사유 분류")
            ...
            if not reason_df.empty:
                fig = px.pie(reason_df, values="종목수", names="사유",
                             title="편출 사유 비중", hole=0.4, height=350,
                             color_discrete_sequence=px.colors.sequential.OrRd_r)
                fig.update_traces(textposition="inside", textinfo="label+value")
                st.plotly_chart(fig, use_container_width=True)

            # 편출 사유 상세 테이블
            st.markdown("#### 종목별 편출 사유 상세")
    ...
    # ──────────────────────────────────────
    with tab5:
        st.markdown("### 분석 기준 (KRX 코스닥 150 지수 방법론)")
        st.caption(
        ...
        **수시변경 (특별 편출)**
        ...
        st.markdown("""
        선정은 **3단계**로 진행되며, 이후 특례/제외 규정이 적용됩니다.
        """)
        ...
        > **150종목 미달 시**
        > - 미선정 종목 중 일평균시가총액이 높은 순으로 추가
        > - 단, 유동성 기준을 충족하는 종목만 추가 가능

        > **150종목 초과 시**
        > - 선정된 종목 중 일평균시가총액이 가장 낮은 종목부터 순서대로 제외
        """)
        ...
        st.warning(
            "**투자 참고용 시뮬레이션입니다. 실제 KRX 정기변경 결과와 차이가 발생할 수 있습니다.**"
        )
        st.markdown("""
        **실제 결과와 차이가 발생할 수 있는 원인:**

        1. **관리종목·투자유의종목 필터 미반영**
           - 현재 시스템은 관리종목/투자유의종목 여부를 자동 판별하지 않음
           - 해당 종목이 심사대상에 포함되어 결과에 영향을 줄 수 있음

        2. **유동주식비율 미반영**
           - KRX는 유동주식비율 10% 미만 종목을 제외하지만, 본 시스템은 이 필터 미적용
           - 대주주 지분이 높은 종목이 과대 반영될 가능성

        3. **GICS 분류 차이**
           - 본 시스템은 WISE Index API의 분류를 사용하며, KRX 자체 분류와 일부 차이 가능
           - 산업군 경계에 있는 종목의 분류가 다를 수 있음

        4. **현재 구성종목 소스**
           - investing.com 크롤링으로 가져오며, 가격 기반 매칭 시 일부 누락 가능성
           - KRX 공식 데이터 대비 1~2종목 차이가 날 수 있음

        5. **빠른 분석 모드의 한계**
           - 당일 스냅샷은 단기 변동에 민감하여 6개월 평균과 괴리 발생
           - 특정 종목의 급등/급락이 결과에 과도하게 반영됨
        """)
    ...
    st.markdown(
        """
        ### 사용 방법
        1. 왼쪽 사이드바에서 **분석 모드**를 선택하세요.
        2. **분석 실행** 버튼을 클릭하면 데이터를 수집하고 분석을 시작합니다.

        #### 5개 분석 섹션
        | 섹션 | 내용 |
        |------|------|
        | **현재 구성 종목** | 현재 코스닥 150 종목, 섹터별 분포, 트리맵 |
        | **향후 예상 구성 종목** | 시뮬레이션 결과, 현재 vs 예상 비교 |
        | **신규 편입 원인 분석** | 편입 예상 종목별 사유, 섹터 내 순위 |
        | **신규 편출 원인 분석** | 편출 예상 종목별 사유, 유동성 분석 |
        | **분석 기준 (방법론)** | KRX 선정 방법론 상세 설명 |
        """
    )
