...
# ── 섹터 분류 (투자 관점 기준) ─────────────────────────
# Industry 키워드 → 섹터 매핑 (순서 중요: 먼저 매칭되는 것이 우선)
SECTOR_MAP = [
    # 2차전지/신재생에너지 — 배터리, 연료전지, 태양광, 풍력
    ("2차전지/신재생에너지", ["일차전지 및 이차전지", "연료용 가스"]),
    # 반도체/전기전자 — 반도체, 전자부품, PCB, 디스플레이, 통신장비
    ("반도체/전기전자",   ["반도체", "전자부품", "통신 및 방송 장비", "디스플레이", "영상 및 음향기기"]),
    # 전력기기 — 전동기, 변압기, 전선, 중전기
    ("전력기기",        ["전동기", "발전기", "전기 변환", "절연선 및 케이블", "기타 전기장비"]),
    # IT/소프트웨어
    ("IT/소프트웨어",    ["소프트웨어", "컴퓨터 프로그래밍", "정보서비스", "데이터베이스",
                       "포털", "호스팅", "인터넷", "자료처리", "기타 정보 서비스"]),
    # 통신 — 유무선 통신사
    ("통신",           ["전기 통신업", "유선통신", "무선통신"]),
    # 자동차/부품
    ("자동차",          ["자동차", "자동차 신품", "자동차용 엔진", "기타 운송장비"]),
    # 조선/기계
    ("조선/기계",        ["선박", "특수 목적용 기계", "일반 목적용 기계", "기계장비"]),
    # 방산/항공
    ("방산/항공",        ["항공기", "우주선", "무기", "탄약", "철도장비"]),
    # 철강/금속/소재
    ("철강/금속",        ["1차 철강", "1차 비철금속", "금속 가공", "금속 주조",
                       "구조용 금속제품", "비금속 광물", "시멘트"]),
    # 화학
    ("화학",            ["기초 화학", "기타 화학", "합성고무", "석유화학", "비료", "농약", "화학섬유"]),
    # 바이오/제약
    ("바이오/제약",      ["의약품", "의료용 기기", "의료용품", "기초 의약", "생물학적",
                       "자연과학 및 공학 연구"]),
    # 금융
    ("금융",            ["금융", "은행", "보험", "증권", "투자", "신탁", "재 보험"]),
    # 건설/인프라
    ("건설",            ["건물 건설", "토목 건설", "건축기술", "전기 및 통신 공사"]),
    # 유통/소비재/음식료
    ("유통/소비재",      ["종합 소매", "무점포 소매", "상품 중개", "전문 도매", "음식료품",
                       "식품", "화장품", "가정용 기기", "임대", "비알코올음료", "알코올음료",
                       "담배", "생활용품", "상품 종합 도매", "기타 상품 전문 소매",
                       "동·식물성 유지", "낙농", "나무제품", "골판지", "종이 상자",
                       "유리 및 유리제품"]),
    # 운송/물류
    ("운송/물류",        ["해상 운송", "항공 운송", "항공 여객", "육상 운송", "도로 화물",
                       "창고", "운송관련 서비스", "운송장비 임대"]),
    # 에너지/유틸리티
    ("에너지/유틸리티",   ["석유 정제", "천연가스", "전기업", "원유", "증기, 냉·온수",
                       "공기조절 공급"]),
    # 미디어/엔터
    ("미디어/엔터",      ["영화", "비디오", "방송프로그램", "오락", "게임",
                       "오디오물 출판", "텔레비전 방송", "광고업"]),
    # 레저/서비스
    ("레저/서비스",      ["여행사", "유원지", "경비", "기타 개인 서비스", "숙박시설"]),
    # 섬유/의복
    ...
    ("플라스틱/고무",     ["플라스틱", "고무제품"]),
    # 정밀기기/측정
    ("정밀기기",         ["측정", "시험", "항해", "정밀기기", "광학"]),
]

# 종목별 보정 (KSIC 분류가 실제 사업과 다른 경우)
TICKER_SECTOR_OVERRIDE = {
    # 2차전지/신재생에너지 (KSIC가 다른 업종으로 분류된 경우)
    "322000": "2차전지/신재생에너지",  # HD현대에너지솔루션 → 태양광 패널
    "336260": "2차전지/신재생에너지",  # 두산퓨얼셀 → 수소연료전지
    "271940": "2차전지/신재생에너지",  # 일진하이솔루스 → 수소탱크
    "112610": "2차전지/신재생에너지",  # 씨에스윈드 → 풍력 타워
    "020150": "2차전지/신재생에너지",  # 롯데에너지머티리얼즈 → 동박(2차전지 소재)
    "086520": "2차전지/신재생에너지",  # 에코프로 → 2차전지 지주
    "383310": "2차전지/신재생에너지",  # 에코프로에이치엔 → 전구체
    "119850": "2차전지/신재생에너지",  # 지엔씨에너지 → ESS/신재생
    # 반도체/전기전자
    "131970": "반도체/전기전자",     # 두산테스나 → 반도체 테스트
    # 방산/항공
    "272210": "방산/항공",         # 한화시스템 → 방산 레이더/SAR
    # 전력기기
    "298040": "전력기기",          # 효성중공업 → 변압기/중전기
    "160190": "전력기기",          # 하이젠알앤엠 → 변압기 부품
    "032820": "전력기기",          # 우리기술 → 전력 계통
    # 유통/소비재 (화장품·뷰티·생활용품 — KSIC "기타 화학제품"에서 이동)
    "192820": "유통/소비재",       # 코스맥스 → 화장품 ODM
    "241710": "유통/소비재",       # 코스메카코리아 → 화장품 ODM
    "090430": "유통/소비재",       # 아모레퍼시픽 → 화장품 브랜드
    "002790": "유통/소비재",       # 아모레퍼시픽홀딩스
    "051900": "유통/소비재",       # LG생활건강 → 생활용품/화장품
    "161890": "유통/소비재",       # 한국콜마 → 화장품 ODM
    "018250": "유통/소비재",       # 애경산업 → 생활용품/화장품
    "278470": "유통/소비재",       # 에이피알 → 뷰티디바이스(메디큐브)
...
PREFERRED_TO_COMMON = {
    ...
    "00680K": "006800",  # 미래에셋증권2우B → 미래에셋증권
    "000155": "000150",  # 두산우 → 두산
    "051915": "051910",  # LG화학우 → LG화학
    "000815": "000810",  # 삼성화재우 → 삼성화재
    "071055": "071050",  # 한국금융지주우 → 한국금융지주
    "00088K": "000880",  # 한화3우B → 한화
    "066575": "066570",  # LG전자우 → LG전자
    "00104K": "001040",  # CJ4우 → CJ
    "003545": "003540",  # 대신증권우 → 대신증권
}


def _classify_sector(industry, ticker=None, name=None):
    """Industry + 종목코드 기반 투자 관점 섹터 분류"""
    # 1) 수동 보정
    if ticker and ticker in TICKER_SECTOR_OVERRIDE:
        return TICKER_SECTOR_OVERRIDE[ticker]

    # 2) Industry 키워드 매칭
    if not industry or pd.isna(industry) or industry == "":
        if name and ("홀딩스" in str(name) or "지주" in str(name)):
            return "지주/홀딩스"
        return "기타"

    # 지주사/경영 컨설팅
    if "회사 본부" in industry or "경영 컨설팅" in industry:
        return "지주/홀딩스"

    for sector_name, keywords in SECTOR_MAP:
        for kw in keywords:
            if kw in industry:
                return sector_name
    return "기타"
...
def load_stock_list(_sector_version=_SECTOR_VERSION):
    ...
    try:
        ...
        desc = pd.DataFrame(columns=["ticker", "industry"])

    df = price_df.merge(desc, on="ticker", how="left")
    df["industry"] = df["industry"].fillna("")
    df = df[(df["close"] > 0) & (df["volume"] > 0)].reset_index(drop=True)

    # 1차: Industry + ticker 기반 분류
    df["sector"] = df.apply(
        lambda r: _classify_sector(r["industry"], r["ticker"], r["name"]), axis=1
    )
    # 2차: 우선주 → 보통주 섹터 따라감
    for pref, common in PREFERRED_TO_COMMON.items():
        common_row = df[df["ticker"] == common]
        if not common_row.empty:
            df.loc[df["ticker"] == pref, "sector"] = common_row.iloc[0]["sector"]
...
KRX 표준산업분류(KSIC) `Industry` 필드를 기반으로 투자 관점의 **24개 섹터**로 재분류합니다.
KSIC 분류가 실제 사업과 다른 종목은 개별 보정하며, 우선주는 보통주 섹터를 따릅니다.

> 반도체/전기전자 · 전력기기 · 2차전지/신재생에너지 · IT/소프트웨어 · 통신 ·
> 자동차 · 조선/기계 · 방산/항공 · 철강/금속 · 화학 · 바이오/제약 · 금융 · 건설 ·
> 유통/소비재 · 운송/물류 · 에너지/유틸리티 · 미디어/엔터 · 레저/서비스 · 섬유/의복 ·
> 부동산 · 플라스틱/고무 · 정밀기기 · 지주/홀딩스 · 기타
