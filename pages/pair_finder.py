...
# 종목명 기반 보정 (KSIC 분류가 실제 사업과 다른 경우)
TICKER_SECTOR_OVERRIDE = {
    "322000": "에너지",          # HD현대에너지솔루션 → 태양광 (반도체X)
...
def _classify_sector(industry, ticker=None):
...
def load_stock_list():
    ...
    try:
        ...
        desc["sector"] = desc.apply(lambda r: _classify_sector(r["industry"], r["ticker"]), axis=1)
    except Exception:
...
KRX 표준산업분류(KSIC) `Industry` 필드를 기반으로 투자 관점의 **23개 섹터**로 재분류합니다.
KSIC 분류가 실제 사업과 다른 종목(예: 태양광 패널 → "반도체 제조업")은 개별 보정합니다.

> 반도체 · 전자/IT부품 · 전력기기 · 2차전지/신에너지 · IT/소프트웨어 · 통신 ·
> 자동차 · 조선/기계 · 방산/항공 · 철강/금속 · 화학 · 바이오/제약 · 금융 · 건설 ·
> 유통/소비재 · 운송/물류 · 에너지 · 미디어/엔터 · 레저/서비스 · 섬유/의복 ·
> 부동산 · 플라스틱/고무 · 정밀기기 · 기타
