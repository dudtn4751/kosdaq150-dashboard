...
설정" 화면과 동일한 목록이고 동일하게 .button__modal 다운로드 버튼을 가지고 있지만
(2026-07-10 확인), 실제 다운로드되는 값 자체는 매월 1일 갱신되는 월 1회 데이터뿐이다
(2026-07-13 실제 스크래핑 결과로 확인 - 전 품목 전체 기간에서 한 품목-월에 2개 이상
행이 있는 경우가 0건이었다). "품목 커스텀 설정" 화면의 진짜 10일/20일 단위 데이터는
이 화면에서 가져올 수 없으므로 scrape_bigfinance_items.py로 그 화면을 별도 방문해야
한다 - 한때 이 화면 방문 하나로 충분하다고 잘못 판단해 통합했다가 되돌렸다.
...
# 그걸 우선 읽고, 팀 원본 호환을 위해 EPIC_FINANCE_ID/PW도 폴백으로 유지한다.
...
def _try_auto_login(page) -> bool:
    ...
    [포팅 2026-07-22] EPIC 로그인 폼 실측 반영: '개인 사용자'/'기업 사용자' 라디오가
    있고 기본값이 개인이다. 이 계정은 기업 계정(API도 /login/enterprise)이므로 먼저
    '기업 사용자'를 선택하지 않으면 개인 로그인으로 처리돼 실패한다. 또 Enter 대신
    '로그인' 제출 버튼 클릭이 더 안정적이다. (실측: 텍스트칸 placeholder='이메일 ID를
    입력하세요', 라디오 id=personal-users/enterprise-users, 버튼 .button__submit)"""
    ...
    try:
        ...
        # 기업 사용자 선택 (라디오는 커스텀 스타일이라 숨김 → 라벨/텍스트 클릭).
        for sel in ("label[for='enterprise-users']", "text=기업 사용자"):
            try:
                ent = page.locator(sel).first
                if ent.count():
                    ent.click(timeout=2000)
                    break
            except PWTimeoutError:
                continue
        ...
        # 제출: '로그인' 버튼 클릭 우선, 실패 시 Enter 폴백.
        try:
            page.locator("button.button__submit, button[type='submit']").first.click(timeout=2000)
        except PWTimeoutError:
...
def scrape_items_and_companies(page) -> tuple[list[dict], list[dict]]:
    ...
    (trade_history_long.csv용, 월말 기준)와 하위 기업(지역) 데이터
    (company_trade_history_long.csv용)를 함께 수집한다.

    이 화면의 다운로드는 월 1회(매월 1일 갱신) 값뿐이라 "품목 커스텀 설정" 화면의
    10일/20일 단위 데이터는 별도로(scrape_bigfinance_items.py) 수집해야 한다
    (2026-07-13 확인).
