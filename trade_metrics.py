...
★ 갱신 일정과 3층위 구조 (2026-08-27 확정 — 이전 '15일 확정 발표' 가정은 폐기)
  - **10일 단위 데이터**: 매월 **1·11·21일** 갱신 (trade_history_decade_long.csv).
    품목 단위만이며 **지역 정보가 없어 기업을 특정할 수 없다.**
  - **월별 데이터 + 기업별**: 매월 **1일에만** 함께 갱신
    (trade_history_long.csv, company_trade_history_long.csv).
    기업 특정은 품목·지역 데이터가 있는 이 월별 갱신에서만 가능하다.
  - 따라서 11·21일 실행은 10일 단위만 갱신하고 월별·기업은 그대로 둔다(누락 아님).
  - 완료월에서 10일 단위의 월말값과 월간 CSV 값은 동일하다(2026-08 전 품목 검증). 월간
    계열의 정본은 trade_history_long.csv이며, 진행월(부분 누계)은 월간 계열에서 제외한다.
"""
