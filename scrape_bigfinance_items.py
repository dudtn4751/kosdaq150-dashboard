...
def scrape_items(page) -> list[dict]:
    ...
    # 이 화면은 "품목 및 지역 커스텀 설정"과 달리 하위 기업/지역 펼치기가 없어 품목명
    # span에 .main-text/.sub-text 구분이 없다 - 그냥 .group__item__name 하나뿐이다
    # (2026-07-13 .auth/debug HTML로 실제 확인).
    top_rows = body.locator(".label__columns table tbody tr").filter(has=page.locator(".group__item__name"))
    ...
    for i in range(n_top):
        name_el = top_rows.nth(i).locator(".group__item__name").first
